...
# ------------------------------------- Main Frame - Compns - Button process fild  ------------------------------
    ...
    def validating_data(self):
        ...
        for row in data_rows:
            try:
                ...
                if cid not in company_ids:
                    ...
                    logger.warning(data_validated['mylistc'])
                    company_ids.add(cid)
                    logger.debug(company_valid.get('Company_id'))
            ...
            try:
                ...
                data_validated['mylistp'].append(particpant_valid.dict())
        ...
        logger.debug(f" Validated cpm list { data_validated['mylistc']}")
        print("\n")
        logger.debug(f" Validated ptcpt list { data_validated['mylistp']}")
                ...
                if participant_type == 2:
                    ...
                    if amount_mandatory:
                        ...
                        if int(money_balance) > 0:
                            ...
                            popup.show_error("Topup amount is 0 for participant ")
                            continue
                    else:
